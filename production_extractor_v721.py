...
请只输出JSON。"""
    ...
    def _process_chunk(self, chunk: List[Dict], doc_type: str, section: str) -> Tuple[List, Dict]:
        """处理单个chunk（用于并行）"""
    ...
    def _extract_sections(self, pages: List[Dict], doc_type: str) -> Tuple[Dict, Dict]:
        ...
        for section, sec_pages in section_buckets.items():
            ...
            if len(sec_pages) > 30:
                ...
                chunks = [sec_pages]
            for chunk in chunks:
                tasks.append((chunk, doc_type, section))
        ...
        if len(tasks) > 1:
            with ThreadPoolExecutor(max_workers=self.MAX_WORKERS) as executor:
                futures = [executor.submit(self._process_chunk, *task) for task in tasks]
            ...
            # 单任务串行
            for task in tasks:
                items, usage = self._process_chunk(*task)
                all_items.extend(items)
                usage_total["prompt_tokens"] += usage.get("prompt_tokens", 0)
                usage_total["completion_tokens"] += usage.get("completion_tokens", 0)
                usage_total["total_tokens"] += usage.get("total_tokens", 0)
